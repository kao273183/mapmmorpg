...
### J2-G　弓箭手套裝 — ✅ 已完成（v0.29.52）

J2 只做了職業本身，忘了裝備線的收藏層：弓箭手系拿不到任何套裝（套裝核心會一直累積卻無處可花），
也沒有任何傳奇弓。本階段補上前者。

| 套裝 | 對應玩法 | 2 件 | 4 件 |
|---|---|---|---|
| **疾風遊獵** | 遊俠：機動放風箏 | 移動速度 +0.5 | 攻擊 +15% |
| **鷹眼獵手** | 神射手：暴擊爆發 | 爆擊率 +6% | 爆擊傷害 +40% |

兩組都沿用既有 stat（全走 `affixV` 管線），不必接新的加成程式；`critDmg` 經 `playerDmg` 也會吃到技能傷害。

**平衡實測**（同一套裝備量兩次，只切換 `setBonusV` 開關，各抽樣 30 萬次 `playerDmg`）：
...
弓系兩組夾在虛空與星火之間，沒有偏強。日耀 +0%、血誓低血正好 +30% 兩個對照組對得上，量測可信。

> 量測踩過兩個坑：①穿上套裝後 `calcStats` 抬高 HP 上限但 `hp` 沒補，角色被判成低血、誤觸血誓的
> 「低於35% HP攻擊+30%」；②`createGear` 的白值是隨機的，開關兩次各自生成裝備等於在比不同的裝。
> 修正成「同一套裝備、補滿血、只切加成開關」後才對得上理論值。純 DPS 模擬的噪音達 ±20%，
> 對這種 10% 級距的差異不可用。

掉落實測：弓系三職各 6000 次 `genGear`，284~303 件（~5%，與其他職同水位），兩組各半，
武器一律是弓，沒有穿不上的件。

**傳奇弓仍未做**——10 把武器類傳奇全是劍或杖，`wpn:'bow'` 一件都沒有，
所以弓箭手系的武器欄不可能是傳奇。留待後續。

### J2-H　傳奇弓 — ✅ 已完成（v0.29.53）

補上 J2-G 留下的另一半：10 把武器類 unique 全是劍或杖，弓系的武器欄永遠不可能是傳奇。

| 弓 | 群系 | 能力 |
|---|---|---|
| **霜羽弓** | 冰霜凍原 | 命中 22% 凍結敵人 |
| **炎羽弓** | 熾熱熔岩 | 命中 32% 點燃（持續燃燒） |
| **貫星弓** | 虛空深淵 | 所有箭矢額外貫穿 1 名敵人 |

前兩把沿用既有的 proc（`applyUniqueWeaponProcs` 掛在 `hitMon` 上，箭矢命中本來就會走到），零新程式。
**貫星弓是新的 `pierceShot` 能力**，在 update.js 的箭矢分支把傳奇弓的額外穿透併進上限；
同時把去重條件從 `pr.pierce` 改成 `pr.hits`——否則被傳奇弓賦予穿透的箭（本身 `pierce=false`）
會缺少去重，同一隻怪被同一支箭重複打。

> **觸發機率的直覺是錯的**：原本想「弓打得比較快，機率要調低」，實測命中頻率是
> 劍士 4.0 次/秒、法師 1.7、弓箭手 1.6——弓其實跟法杖差不多，近戰扇形反覆掃到多目標才是最高的。
> 所以數值直接對齊法杖，沒有調低。

`dungeon-i1-smoke` 補推導式檢查：每個基礎職都要有傳奇武器、武器類 unique 的 `wpn` 必須對得上
該職武器欄、非武器不得有 `wpn`。這種缺口不會報錯，只會讓一整個職業系少一條收藏線。

### J2-I　修正箭矢傷害雙重結算 + 重新調校 — ✅ 已完成（v0.29.54）

做傳奇弓的驗證時發現的：**箭矢每次命中都結算兩次傷害**，從 J2-A（v0.29.47）上線起就存在。

update.js 的投射物迴圈對所有 kind 先跑一次通用的 `hitMon(m, r.d, r.crit)`，
而 J2-A 為箭矢加的專屬分支（要吃穿透遞減／專注／迅步／額外爆擊）又自己算了一次、再打一次。
旁邊的 `elem`／`shadow` 分支只加狀態、傷害交給通用那次，正好是對照組——不一致就是徵兆。

發現的線索是傳奇弓的觸發率：標示 22% 的霜羽弓實測 38%、標示 32% 的炎羽弓實測 51%，
兩個都正好等於 `1-(1-p)²`——只有「觸發判定跑了兩次」會是這個數。

**修正**：通用那次加上 `if (pr.kind !== 'arrow')`。`r` 仍要算，因為 `elem`／`shadow` 的
持續傷害依它縮放。

**重新調校**：修好之後弓系傷害掉到約一半（弓箭手單體 206→94），因為 J2-B/J2-F 是對著
雙倍輸出調的，寫在程式裡的倍率其實只有設計值的一半。所有箭矢倍率 ×2 後回到目標值：

| | 修正前(雙倍) | 修正後 | ×2 調校後 | 對目標 |
|---|---|---|---|---|
| 弓箭手 單體／群體5 | 206 / 1546 | 94 / 755 | 210 / 1435 | 102% / 93% |
| 遊俠 | 183 / 1652 | 145 / 1378 | 193 / 1668 | 105% / 101% |
| 神射手 | 258 / 1798 | 132 / 936 | 250 / 1686 | 97% / 94% |

六個數據點全落在 93–105%（量測本身的變異約 ±10%），不需要再逐職微調。
對照組劍士 189／1465、法師 275／3242 與先前一致。傳奇弓觸發率也回到標示值（實測 20%／34%）。

`skill-fx-smoke` 補上不變量：**任何自己呼叫 `hitMon` 的 kind 分支，都必須被通用那次的守衛排除**。
用大括號配對界定命中區塊（先前用正則會吃過頭，把後面隕石的 `hitMon` 誤算進 `ice` 分支）。
突變測試確認拿掉守衛會被抓到。

> J1-F 的「進階職效率不得離群」測試抓不到這個——它用靜態正則抽 `skillDmg(X * t.dmg)`，
> 而投射物技能的倍率寫在 `projs.push` 的 `mult` 欄位裡，本來就抽不到（該測試自己有註明這個限制）。
> 這類「數值對、但結算次數錯」的 bug 只有實跑量測會現形。
