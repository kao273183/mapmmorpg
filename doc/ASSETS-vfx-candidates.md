...
Pixel Art Spells 全是法術投射物，**一張近戰斬擊都沒有**，所以劍士／狂戰士／聖騎士要另外找。

### 現況缺口

| 技能 | 目前用的圖集 | 問題 |
|---|---|---|
| 揮砍・裂顱斬・血怒斬・聖印斬 | `slashBeam` ×4 | **四個技能共用同一張刀光** |
| 旋風斬 | `rune` | 是**魔法符文**，法師系的素材，完全不對味 |
| 狂暴・戰吼 | `groundBurst` ×2 | 兩個自我增益技撞臉 |
| 震地波・制裁光錘 | `groundImpact` ×2 | 撞臉（制裁光錘可改用已下載的 `splash` 染金，免下載） |
| 突進斬 | `smoke` + `teleport` | 尚可，但沒有衝刺軌跡感 |

### 1. Weapon Slash - Effect — Cethiel｜**CC0**｜最推薦
<https://opengameart.org/content/weapon-slash-effect>

- **5 種不同的揮擊／斬擊動畫**，另附 4 種顏色變體
- 正好解決「四個技能共用一張」——一種分一個技能
- 顏色變體對我們是額外的，因為 `tintedSkillVfx()` 已經能染色

### 2. Pixel art sword slash effect — tbbk｜**CC0**
<https://opengameart.org/content/pixel-art-sword-slash-effect>

- 單張 sprite sheet，每格 **64×47**（非正方形，接入時要留意 `frame` 欄位目前假設正方形）
- 直接下載：<https://opengameart.org/sites/default/files/pixel_art_sword_slash_sprites.png>
- 作者：「Credits appreciated but not needed.」

### 3. Explosion effects and more — Soluna Software｜**授權需確認**
<https://opengameart.org/content/explosion-effects-and-more>

- 含**光環（aura）**與爆炸，適合狂暴／戰吼的自我增益表現
- ⚠️ 頁面同時掛 **CC-BY 3.0 與 CC0 兩個標章**，未載明哪些檔案適用哪個。用之前要弄清楚，或直接當 CC-BY 處理（署名 Soluna Software）

### 4. Whirlwind — Spring Spring｜**CC-BY 3.0 / CC-BY-SA 3.0（需署名）**
...
### 5. 不適用
- [Attack animation](https://opengameart.org/content/attack-animation)（bellow, CC-BY 3.0）是**角色揮劍的 sprite**，不是特效。本專案角色是程式繪製的像素圖，用不上。

### 還可以挖
- [05. Pixel Art - Effects](https://opengameart.org/content/05-pixel-art-effects) 合輯
- [Explosions, Bullets, Fire etc (Pixel art)](https://opengameart.org/content/explosions-bullets-fire-etc-pixel-art) 合輯（`pixel-art-sword-slash-effect` 就收在裡面）

### 建議下載順序

只拿 CC0 的兩組就能解掉最嚴重的撞臉，且不必加致謝頁：

```bash
# 1) Weapon Slash - Effect（Cethiel, CC0）— 到頁面下載後解壓到：
#    assets/source/weapon-slash-cethiel/
# 2) Pixel art sword slash effect（tbbk, CC0）— 可直接抓：
curl -L -o "assets/source/sword-slash-tbbk/pixel_art_sword_slash_sprites.png" \
  https://opengameart.org/sites/default/files/pixel_art_sword_slash_sprites.png
```

旋風斬與增益光環的候選都要署名，建議一起決定要不要加「素材致謝」頁再處理。
