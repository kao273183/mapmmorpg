...
const warChars = spriteChars('WAR'), mageChars = spriteChars('MAGE');
...
for (const id of Object.keys(api.COLOR_DEFS)) {
  ...
  assert.ok(map && map.r && map['4'], id + ' 的對照表必須同時涵蓋劍士系(r)與法師系(4)');
  for (const ch of Object.keys(map)) {
    assert.ok(warChars.has(ch) || mageChars.has(ch), id + ' 重上色了精靈圖沒用到的字元 ' + ch);
...
const playerDraws = renderSrc.match(/drawSprite\((?:baseClassOf\(p\.cls\)|p\.cls)[^\n]*\)/g) || [];
