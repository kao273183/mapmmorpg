...
function applyUniqueWeaponProcs(m, d, crit) { // 傳奇武器命中觸發（凍結/緩速/吸血/連鎖）
  ...
  for (const pw of def.powers) {
    if (pw.type === 'freeze') { if (Math.random() < pw.chance && !(m.ccT > 0)) { m.freezeT = Math.max(m.freezeT || 0, pw.dur); m.ccT = pw.dur + 72; num(m.x, m.y - m.h - 20, '凍結', '#d8f4ff'); } } // CC 遞減：凍結後短暫免疫，避免永久鎖控
