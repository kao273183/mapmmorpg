...
const GAME_UPDATE_NOTES = [
  ...
  {
    version:'0.29.47', date:'2026-07-24', title:'新職業：弓箭手（J2-A）',
    items:['新增第三個基礎職「弓箭手」：綠羽兜帽的遊俠，遠程持續射擊。選角頁現在有三張基礎職卡。','基礎技能「射擊」射出一箭，命中造成傷害；天賦分支精準（穿透/受傷加成）或速射（雙箭/箭速）。','用真正的弓當武器（短弓→風暴弓五階稀有度），箭矢與弓為 CC0 素材（DCSS，與現有裝備同源）。','進階轉職（遊俠／神射手）與更多技能於後續版本開放。']
  },
  {
    version:'0.29.46', date:'2026-07-24', title:'斬擊顏色隨技能練度變化',
    items:['劍士系斬擊的顏色現在會反映技能練到什麼程度：配點越深越亮，未練偏暗沉、滿級最鮮明。','兩條天賦分支走不同色系——例如裂顱斬撕裂是血紅、狂亂轉烈焰橙；制裁審判是金、聖療轉治療綠。','貼圖染色與細弧線用同一個顏色，整記斬擊一起變。']
  },
  {
    version:'0.29.45', date:'2026-07-24', title:'劍士系斬擊特效改版',
    items:['劍士系原本四個技能共用同一張刀光、旋風斬還用著法師的魔法符文。現在改用手繪斬擊特效，每個技能都不一樣。','揮砍是鋼白粗月牙、裂顱斬是血紅雙斬、旋風斬是金色環形斬、聖印斬是金色細月牙、血怒斬與制裁是斜刺、突進斬是青色斜掃線。','特效系統加入二維網格與非正方形圖集支援。新素材為 CC0（Weapon Slash - Effect by Cethiel），只用一種顏色，其餘靠引擎染色。']
  },
  {
    version:'0.29.44', date:'2026-07-23', title:'導入新的技能特效素材',
    items:['元素飛彈、暗影箭、聖盾壁壘、疫咒改用新的手繪特效圖集，不再和既有技能共用同一套動畫。','特效系統加入染色：同一組素材可依元素或職業換色，且保留原本的明暗層次。元素飛彈的火／冰／雷就是同一組圖染出來的。','特效圖集不再限定 72×72，各組可宣告自己的來源尺寸。','新素材為 CC0（Pixel Art Spells by DevWizard），來源與授權已登記於專案內。']
  },
  {
    version:'0.29.43', date:'2026-07-23', title:'進階職基本技能的專屬特效（J1-G 補完）',
    items:['元素飛彈與暗影箭原本被畫成火球（新投射物沒有繪製分支）。現在元素飛彈是依火／冰／雷換色的旋轉稜形彈，暗影箭是紫黑核心加拖曳殘影。','近戰刀光改為每個技能有自己的顏色、半徑與弧度：揮砍白色寬弧、裂顱斬血紅粗弧（並追加第二道反刀）、聖印斬金色細弧、突進斬藍白短弧。','修正刀光顏色會從上一個施放的技能殘留下來的問題。']
  },
  {
    version:'0.29.42', date:'2026-07-23', title:'進階職專屬基本技能（J1-G）',
    items:['四個進階職不再沿用基礎職的普攻，各自有專屬基本技能取代它：狂戰士「裂顱斬」血越低威力越高、聖騎士「聖印斬」每第三擊回血、元素師「元素飛彈」火冰雷輪替各帶效果、咒術師「暗影箭」直傷較低但疊加腐蝕。','四個新技能都有自己的天賦分支，並排在技能樹的根節點；技能總數維持 7 個，技能樹版面不變。','持續輸出實測對齊同系基礎職：狂戰士 +8%（低血時 +44%）、聖騎士 −7%（換來治療）、元素師 +13%、咒術師 +6%。','既有存檔的進階職出戰欄會自動換成新的基本技能，不需手動調整。']
  },
  {
    version:'0.29.41', date:'2026-07-23', title:'職業線平衡與回歸（J1-F）',
    items:['血怒斬下修：傷害倍率 2.4→1.9、MP 6→11，HP 消耗提高到最大HP的 8–10%。原本每 MP 效率是同系最強技能的 3.2 倍，換血代價卻幾乎不存在。','連鎖風暴上修：起始倍率 1.7→2.8、跳躍 4→5 段、衰減 0.82→0.85、MP 22→18。原本是全遊戲第二貴的 MP 配上最低輸出，沒有理由選它。','四個進階職加入固定種子基準檔，平衡統計也改為涵蓋所有職業；修正基準裝備對進階職標錯職業導致穿不上的問題。','確認精通與外觀完全不影響戰力，並確認 J1 之前的舊存檔可無痛升級：技能、天賦、出戰欄完整保留，進階職維持未解鎖。']
  },
  {
  ...
  if (gameState === 'select') {
    ...
    if (menuTab === 'stash') {
      ...
      for (const b of stashActBtns) {
        ...
        if (!sel) return;
        if (b.act === 'equip') {
          if (!gearUsableByClass(sel, chosenCls)) { menuMsg = { text:'此裝備限 ' + classDisplayName(sel.cls) + ' 使用', color:'#ff8a8a', t:180 }; playSfx('uiError'); return; }
